...
def get_request_args(path, action, swagger_parser):
    ...
    Args:
        ...
        swagger_parser: instance of OASParser.
    Returns:
    ...
    if path in swagger_parser.paths.keys() and action in swagger_parser.paths[path].keys():
        ...
        if 'parameters' in operation_spec.keys():
            for param_name, param_spec in operation_spec['parameters'].items():
                request_args[param_name] = swagger_parser.get_example_from_prop_spec(
                    param_spec)
...
def validate_definition(swagger_parser, valid_response, response):
    ...
    if ((not isinstance(response, dict) or not isinstance(valid_response, dict)) and
        ...
        valid_definition = swagger_parser.get_dict_definition(
            valid_response, get_list=True)
        actual_definition = swagger_parser.get_dict_definition(
...
def get_url_body_from_request(action, path, request_args, swagger_parser):
    ...
    url, body, query_params, headers, files = parse_parameters(
        path, action, path, request_args, swagger_parser)
    ...
    for action in _HTTP_METHODS:
        for operation in operation_sorted[action]:
            ...
            url, body, headers, files = get_url_body_from_request(
                action, path, request_args, swagger_parser)
            ...
            if response.status_code is not 404:
                ...
                body_req = swagger_parser.get_send_request_correct_body(
                    path, action)

                try:
                    response_spec = swagger_parser.get_request_data(
                ...
                if response.status_code in response_spec.keys():
                    ...
                    validate_definition(
                        swagger_parser, response_spec['default'], response_json)
                else:
