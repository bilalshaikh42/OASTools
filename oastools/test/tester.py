...
Adapted from Swagger_Tester (MIT Licence) to work with openapi 3
https://github.com/Trax-air/swagger-tester/blob/master/swagger_tester/swagger_tester.py
"""
...
from oastools.parse import OASParser
...
def get_request_args(path, action, oas_parser):
    """Get request args from an action and a path.
    Args:
        path: path of the action.
        action: action of the request(get, delete, post, put).
        oas_parser: instance of OASParser.
    Returns:
        A dict of args to transmit to bravado.
    """
    request_args = {}
    if path in oas_parser.paths.keys() and action in oas_parser.paths[path].keys():
        operation_spec = oas_parser.paths[path][action]

        if 'parameters' in operation_spec.keys():
            for param_name, param_spec in operation_spec['parameters'].items():
                request_args[param_name] = oas_parser.get_example_from_prop_spec(
                    param_spec)

    return request_args


def validate_definition(oas_parser, valid_response, response):
    """Validate the definition of the response given the given specification and body.
    Args:
        oas_parser: instance of swagger parser.
    ...
    if ((not isinstance(response, dict) or not isinstance(valid_response, dict)) and
        ...
        valid_definition = oas_parser.get_dict_definition(
            valid_response, get_list=True)
        actual_definition = oas_parser.get_dict_definition(
            response, get_list=True)
        assert len(set(valid_definition).intersection(actual_definition)) >= 1


def parse_parameters(url, action, path, request_args, oas_parser):
    ...
    if path in oas_parser.paths.keys() and action in oas_parser.paths[path].keys():
        ...
        for parameter_name, parameter_spec in operation_spec['parameters'].items():
            if parameter_spec['in'] == 'body':
                ...
                url = url.replace('{{{0}}}'.format(
                    parameter_name), str(request_args[parameter_name]))
            elif parameter_spec['in'] == 'query':
                if isinstance(request_args[parameter_name], list):
                    query_params[parameter_name] = ','.join(
                        request_args[parameter_name])
                else:
                    query_params[parameter_name] = str(
                        request_args[parameter_name])
                ...
                header_value = header_value or parameter_spec.get(
                    'default', '')
                headers += [(parameter_spec['name'], str(header_value))]
    return url, body, query_params, headers, files


def get_url_body_from_request(action, path, request_args, oas_parser):
    ...
    """
    url, body, query_params, headers, files = parse_parameters(
        path, action, path, request_args, oas_parser)
...
def get_method_from_action(client, action):
    ...
    """
    error_msg = "Action '{0}' is not recognized; needs to be one of {1}.".format(
        action, str(_HTTP_METHODS))
...
def swagger_test_yield(oas_spec=None, app_url=None, authorize_error=None,
    ...
    operations = oas_parser.operation.copy()
    operations.update(oas_parser.generated_operation)
    ...
    for action in _HTTP_METHODS:
        for operation in operation_sorted[action]:
            ...
            request_args = get_request_args(path, action, oas_parser)
            url, body, headers, files = get_url_body_from_request(
                action, path, request_args, oas_parser)
            ...
            if oas_spec is not None and app_url is None:
                ...
                if app_url.endswith(oas_parser.base_path):
                    base_url = app_url[:-len(oas_parser.base_path)]
                else:
                    base_url = app_url
            ...
            if (action in authorize_error and path in authorize_error[action] and
                ...
                logger.info(u'Got expected authorized error on {0} with status {1}'.format(
            ...
            if response.status_code is not 404:
                ...
                if response.status_code in response_spec.keys():
                    validate_definition(
                        oas_parser, response_spec[response.status_code], response_json)
                elif 'default' in response_spec.keys():
                    validate_definition(
                        oas_parser, response_spec['default'], response_json)
                else:
                ...
                # 404 => Postpone retry
                # Already postponed => raise error
                if {'action': action, 'operation': operation} in postponed:
                    raise Exception(
                        u'Invalid status code {0}'.format(response.status_code))
